...
export async function getEffectiveCapabilities(
  ...
  if (ADMIN_STATUSES.has(await resolveMembershipStatus(userId)))
    return new Set(["*"]);
  ...
  for (const assignment of assignments.documents) {
    ...
    const assignmentScope = String(row.scopeType || "global");
...
export async function hasPower(
  ...
  try {
    if (ADMIN_STATUSES.has(await resolveMembershipStatus(userId))) return true;
  } catch {
...
export async function getAccessSummary(userId: string, knownStatus?: string) {
  const [status, capabilities] = await Promise.all([
    knownStatus ?? resolveMembershipStatus(userId),
