...
export function getSpotsLeft(capacity?: number, registered?: number): number | null {
  if (typeof capacity !== "number" || capacity <= 0) return null;
  return Math.max(0, capacity - (registered ?? 0));
}

export function getRegistrationPercentage(capacity?: number, registered?: number): number {
...
export function getAvatarUrl(avatar?: string | null, name?: string): string {
  ...
  const initial = encodeURIComponent((name ?? "M").trim().charAt(0).toUpperCase() || "M");
