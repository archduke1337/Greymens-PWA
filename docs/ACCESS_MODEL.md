...
|---|---|---|
| `PUBLIC_READ_TABLES` | anyone may read | events, projects, departments, designations, powers, event_types, blogs, sponsors, gallery |
...
| `SERVER_ONLY_TABLES` | server API key only | role_templates, role_assignments, user_roles, authorized_activities, incident_reports, governance_records, office_assignments, audit_logs, notifications |
...
`lib/*.ts` service modules (`profiles`, `memberships`, `departments`,
`designations`, `powers`, `tickets`, `resources`, `notifications`, `audit`,
`gallery`, `blog`, `sponsors`, `database`) still call the Appwrite **browser**
SDK directly, and several pages call those services. Until those call sites are
migrated, the permission classes above cannot be applied to a live project
without breaking the affected screens.
