# Greymens — Database Schema Design
...
> Note: there is no `resource_access` table. Resource visibility is computed
> in `GET /api/resources` from the resource row itself (`category`,
> `departmentId`, `requiredRole`) plus the caller's `user_departments` rows —
> no separate access table is provisioned, and `scripts/setup-appwrite.js` is
> authoritative over this document when they disagree.
