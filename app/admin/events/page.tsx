...
export default function AdminEventsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.description?.trim())
      missing.push("description (Basic Info tab)");
    ...
    if (!formData.location?.trim())
      missing.push("location (Date & Location tab)");
    if (!formData.organizerName?.trim())
      missing.push("organizer name (Organizer tab)");
    if (missing.length > 0) {
    ...
    if (
      ...
      !formData.organizerAvatar.startsWith("http")
    ) {
      toast.error(
        "Please enter a valid organizer avatar URL (must start with http:// or https://)",
      );
    ...
    if (discountPrice !== null && discountPrice >= price) {
      toast.error(
        "Discount price must be less than the regular price (Pricing tab)",
      );
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to save event"));
      ...
      logError("Error saving event:", message);
  ...
  const handleLifecycle = async (
    ...
    const label =
      action === "approve"
        ? "Approve this event?"
        : "Publish this event? It will become publicly visible.";
  ...
  const handleLifecycleConfirm = async (
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, `Unable to ${action} event`));
      await loadEvents();
      toast.success(
        action === "approve"
          ? "Event approved."
  ...
  const handleDelete = async (eventId: string) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to delete event"));
      await loadEvents();
  ...
  const handleDeletePastEvents = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to delete past events"));
      const count = payload?.deleted ?? 0;

      await loadEvents();
      toast.success(`${count} past events deleted successfully!`);
    } catch (error) {
      logError("Error deleting past events:", error);
                      ...
                      <TableRow key={event.$id}>
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <Avatar className="flex-shrink-0" size="lg">
                              <AvatarImage
                                alt={event.title}
                                src={event.image}
                              />
                          ...
                          <div className="flex gap-1 md:gap-2">
                            {(event.status === "draft" ||
                              ...
                              <Button
                                ...
                                variant="secondary"
                                onPress={() =>
                                  handleLifecycle(event.$id!, "approve")
                                }
                              >
                                Approve
                              </Button>
                            ...
                            {event.status === "approved" && (
                              <Button
                                ...
                                variant="secondary"
                                onPress={() =>
                                  handleLifecycle(event.$id!, "publish")
                                }
                              >
                                Publish
                              </Button>
                ...
                <form onSubmit={handleSubmit}>
                  <ModalHeader className="flex flex-col gap-1 border-b pb-4">
                    ...
                    <p className="text-sm text-default-500 font-normal">
                      ...
                      {editingEvent ? "update" : "create"} an event
                    </p>
                  </ModalHeader>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="p-3 bg-primary/10 rounded-lg">
                              <p className="text-xs text-primary">
                                Tip: use free image hosting services like Imgur,
                                Cloudinary, or Unsplash for reliable image URLs
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="space-y-6 pt-4">
                          ...
                          <div className="p-4 bg-muted rounded-xl">
                            ...
                            <ul className="text-sm text-muted-foreground space-y-1">
                              ...
                              <li>
                                • Add discount price for early bird offers
                              </li>
                              <li>
