...
export default function AdminMembershipRejectedPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const response = await fetch("/api/admin/membership?status=rejected", {
        cache: "no-store",
      });
      ...
      if (!response.ok)
        throw new Error(
          readApiError(payload, "Failed to load rejected applications"),
        );
      ...
      logError("Error loading rejected applications:", error);
      toast.error(
        getErrorMessage(error) || "Failed to load rejected applications",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (authLoading || loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div
          aria-label="Loading rejected applications"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-default-500 text-sm md:text-base ml-11">
            ...
            {applications.length !== 1 ? "s" : ""} rejected
          </p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold tabular-nums">
                  {
                    applications.filter((a) => a.application.rejectionReason)
                      .length
                  }
                </p>
              </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableScrollContainer>
                <TableContent
                  aria-label="Rejected applications table"
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center gap-3">
                                  <Avatar className="h-10 w-10 shrink-0">
                                    <AvatarImage
                                      alt={
                                        accountNames[item.application.userId] ||
                                        item.profile?.urn ||
                                        "Applicant"
                                      }
                                      src={
                                  ...
                                  <div className="min-w-0">
                                    <p className="font-semibold text-sm truncate">
                                      {accountNames[item.application.userId] ||
                                        item.profile?.urn ||
                                        item.application.userId.slice(0, 12)}
                                    </p>
                                ...
                                <Button
                                  ...
                                  onPress={() =>
                                    setExpandedId(
                                      isExpanded ? null : item.application.$id!,
                                    )
                                  }
                                >
            ...
            <ModalDialog className="sm:max-w-2xl">
              <ModalHeader className="flex flex-col gap-1 border-b pb-4">
                <h2 className="text-xl font-bold">
                  {detailsApp
                    ? accountNames[detailsApp.application.userId] ||
                      "Applicant details"
                    : "Applicant details"}
                </h2>
