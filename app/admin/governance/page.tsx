...
import { Button, Card, CardContent, CardHeader, Chip, Input, Label, ListBox, Select, Spinner, TextArea, TextField } from "@heroui/react";
...
export default function GovernanceAdminPage() {
  ...
  if (authLoading || loading) return <div className="flex min-h-[50vh] items-center justify-center"><div className="text-center space-y-4" role="status" aria-label="Loading governance records"><Spinner size="lg" /><p className="text-default-500">Loading governance records...</p></div></div>;
  if (!user || !isRole("admin")) return null;

  return (
    <main className="mx-auto max-w-6xl space-y-8 px-4 py-12">
      <div className="flex items-center gap-4"><Button variant="secondary" onPress={() => router.back()}><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button><div><h1 className="text-3xl font-bold">Governance records</h1><p className="text-muted">Minutes, resolutions, amendments, handovers, and continuity records.</p></div></div>
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <Card><CardHeader className="flex-row items-center gap-3"><Plus className="h-5 w-5 text-accent" /><h2 className="text-xl font-bold">Create record</h2></CardHeader><CardContent><form className="space-y-4" onSubmit={createRecord}>
          ...
          <div className="grid grid-cols-2 gap-3"><div><Select fullWidth value={form.visibility} onChange={(value) => setForm({ ...form, visibility: String(value ?? form.visibility) })}><Label>Visibility</Label><Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger><Select.Popover><ListBox><ListBox.Item id="public" textValue="Public">Public<ListBox.ItemIndicator /></ListBox.Item><ListBox.Item id="members" textValue="Members">Members<ListBox.ItemIndicator /></ListBox.Item><ListBox.Item id="restricted" textValue="Restricted">Restricted<ListBox.ItemIndicator /></ListBox.Item></ListBox></Select.Popover></Select></div><p className="self-center text-xs text-muted">New records start as drafts and are approved explicitly.</p></div>
          <Button type="submit" variant="primary" isPending={saving}>Save record</Button>
        </form></CardContent></Card>
        <div className="space-y-4"><h2 className="flex items-center gap-2 text-xl font-bold"><BookOpen className="h-5 w-5" /> Existing records</h2>{records.length === 0 ? <Card><CardContent className="p-8 text-center text-muted">No governance records yet.</CardContent></Card> : records.map((record) => { const expanded = expandedId === record.$id; return (<Card key={record.$id}><CardContent className="space-y-2 p-5"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold">{record.title}</h3><div className="flex gap-2"><Chip size="sm" variant="soft">{record.recordType}</Chip><Chip size="sm" color={record.status === "approved" ? "success" : record.status === "archived" ? "default" : "warning"}>{record.status}</Chip></div></div><p className={`${expanded ? "" : "line-clamp-3"} whitespace-pre-wrap text-sm text-muted`}>{record.body}</p><p className="text-xs text-muted">{record.meetingDate || record.updatedAt || "No date"} · {record.visibility}</p><div className="flex flex-wrap gap-2 pt-1"><Button size="sm" variant="secondary" onPress={() => setExpandedId(expanded ? null : record.$id)}>{expanded ? "Show less" : "Read full record"}</Button>{record.status === "draft" && <Button size="sm" variant="secondary" isPending={transitionId === record.$id} onPress={() => transitionRecord(record.$id, "approved")}>Approve</Button>}{record.status === "approved" && <Button size="sm" variant="secondary" isPending={transitionId === record.$id} onPress={() => transitionRecord(record.$id, "archived")}>Archive</Button>}{record.status === "archived" && <Button size="sm" variant="secondary" isPending={transitionId === record.$id} onPress={() => transitionRecord(record.$id, "draft")}>Reopen as draft</Button>}</div></CardContent></Card>); })}</div>
      </div>
    </main>
  );
}
