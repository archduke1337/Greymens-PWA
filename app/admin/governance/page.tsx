...
export default function GovernanceAdminPage() {
  ...
  const [form, setForm] = useState({
    recordType: "minute" as RecordType,
    title: "",
    body: "",
    meetingDate: "",
    visibility: "members",
  ...
  const loadRecords = useCallback(async () => {
    try {
      ...
      toast.error("Could not load governance records");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createRecord = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to create record"));
  ...
  const transitionRecord = async (
    ...
    const label =
      status === "approved"
        ? "Approve this record? It becomes the official version."
        : status === "archived"
          ? "Archive this record?"
          : "Reopen this record as a draft?";
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to update record"));
      toast.success(
        status === "approved"
          ? "Record approved"
          : status === "archived"
            ? "Record archived"
            : "Record reopened as draft",
      );
      await loadRecords();
    } catch (error) {
      toast.error(
  ...
  if (authLoading || loading)
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div
          aria-label="Loading governance records"
          className="text-center space-y-4"
          role="status"
        >
          <Spinner size="lg" />
          <p className="text-default-500">Loading governance records...</p>
        </div>
      </div>
    );
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-8 px-4 py-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Governance records</h1>
          <p className="text-muted">
            Minutes, resolutions, amendments, handovers, and continuity records.
          </p>
        </div>
      </div>
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <Card>
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={createRecord}>
              <div>
                <Select
                  ...
                  onChange={(value) =>
                    setForm({
                      ...form,
                      recordType: String(value ?? "") as RecordType,
                    })
                  }
                >
              ...
              <TextField variant="secondary">
                <Label>Meeting / effective date</Label>
                <Input
                  type="date"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Select
                    ...
                    onChange={(value) =>
                      setForm({
                        ...form,
                        visibility: String(value ?? form.visibility),
                      })
                    }
                  >
                ...
                <p className="self-center text-xs text-muted">
                  New records start as drafts and are approved explicitly.
                </p>
              </div>
        ...
        <div className="space-y-4">
          <h2 className="flex items-center gap-2 text-xl font-bold">
            <BookOpen className="h-5 w-5" /> Existing records
          </h2>
          {records.length === 0 ? (
            <Card>
              ...
              return (
                <Card key={record.$id}>
                  <CardContent className="space-y-2 p-5">
                    ...
                    <p
                      className={`${expanded ? "" : "line-clamp-3"} whitespace-pre-wrap text-sm text-muted`}
                    >
                      {record.body}
                    </p>
                    <p className="text-xs text-muted">
                      {record.meetingDate || record.updatedAt || "No date"} ·{" "}
                      {record.visibility}
                    </p>
                    <div className="flex flex-wrap gap-2 pt-1">
                      ...
                      >
                        {expanded ? "Show less" : "Read full record"}
                      </Button>
                      {record.status === "draft" && (
                        <Button
                          isPending={transitionId === record.$id}
                          size="sm"
                          variant="secondary"
                          onPress={() =>
                            transitionRecord(record.$id, "approved")
                          }
                        >
                          Approve
                        </Button>
                      ...
                      {record.status === "approved" && (
                        <Button
                          ...
                          variant="secondary"
                          onPress={() =>
                            transitionRecord(record.$id, "archived")
                          }
                        >
                          Archive
                        </Button>
                      ...
                      {record.status === "archived" && (
                        <Button
                          ...
                          size="sm"
                          variant="secondary"
                          onPress={() => transitionRecord(record.$id, "draft")}
                        >
                          Reopen as draft
                        </Button>
                      )}
