...
function ticketCode() {
  return `MM-${crypto.randomUUID().replace(/-/g, "").slice(0, 8).toUpperCase()}`;
}
...
async function issueTicket(
  ...
  return databases.createDocument(DATABASE_ID, COLLECTIONS.TICKETS, ID.unique(), {
    userId: registration.userId,
    eventId: registration.eventId,
    registrationId: registration.$id,
    ticketCode: code,
    qrData: JSON.stringify({ ticketCode: code, eventId: registration.eventId }),
    status: "issued",
