...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const queries =
      type && RECORD_TYPES.has(type)
        ? [
            Query.equal("recordType", [type]),
    ...
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTIONS.GOVERNANCE_RECORDS,
      queries,
    );

    return ok({ records: response.documents, total: response.total });
  } catch (error) {
    logError("Governance record list error:", error);
...
export async function POST(request: NextRequest) {
  ...
  if (
    !consumeRateLimit(
      `governance-mutate:${authenticated.user.$id}`,
      60,
      10 * 60 * 1000,
    ).allowed
  ) {
  ...
  try {
    ...
    const recordType =
      typeof body.recordType === "string" ? body.recordType.trim() : "";
    const title = typeof body.title === "string" ? body.title.trim() : "";
    const recordBody = typeof body.body === "string" ? body.body.trim() : "";
    const visibility =
      typeof body.visibility === "string" ? body.visibility.trim() : "members";
    const status =
    ...
    const meetingDate =
      typeof body.meetingDate === "string" && body.meetingDate.trim()
        ? body.meetingDate.trim().slice(0, 30)
        : undefined;

    if (meetingDate !== undefined && !isIsoDate(meetingDate))
      return fail("VALIDATION", "Meeting date must be a valid date", 400);
    ...
    logError("Governance record create error:", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      (!data.title.trim() || data.title.length > 255)
    )
      return fail("VALIDATION", "Invalid title", 400);
    if (
      typeof data.body === "string" &&
      (!data.body.trim() || data.body.length > 65535)
    )
      return fail("VALIDATION", "Invalid body", 400);
    if (
      data.visibility !== undefined &&
      (typeof data.visibility !== "string" ||
        !VISIBILITIES.has(data.visibility))
    )
      return fail("VALIDATION", "Invalid visibility", 400);
    if (
      data.status !== undefined &&
      (typeof data.status !== "string" || !STATUSES.has(data.status))
    )
      return fail("VALIDATION", "Invalid status", 400);
    const { databases } = createServerDatabases();
    ...
    logError("Governance record update error:", error);
