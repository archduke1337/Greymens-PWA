...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logError("Editorial blog list error:", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (
      !blogId ||
      !["approve", "reject", "publish", "feature"].includes(String(action))
    ) {
    ...
    if (
      existing.authorId === authenticated.user.$id &&
      ["approve", "publish", "feature"].includes(String(action))
    ) {
      return fail(
        "FORBIDDEN",
        "Authors cannot approve, publish, or feature their own work",
        403,
      );
    }
    const data =
    ...
    logError("Editorial blog update error:", error);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    logError("Admin blog delete error:", error);
