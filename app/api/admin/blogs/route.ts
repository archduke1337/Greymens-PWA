...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (existing.authorId === authenticated.user.$id && ["approve", "publish"].includes(String(action))) {
      return fail("FORBIDDEN", "Authors cannot approve or publish their own work", 403);
    }
