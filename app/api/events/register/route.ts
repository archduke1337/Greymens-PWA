...
function ticketCode(): string {
  // 48-bit entropy (12 hex chars) vs previous 32-bit; collision-checked below.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let ticket = null;
    if (status === "approved") {
      // Unique ticket code with clash check (unique index on ticketCode is the
      // final guard; this loop avoids retry storms).
      let ticketCodeValue = "";
      for (let attempt = 0; attempt < 5; attempt += 1) {
        const candidate = ticketCode();
        const clash = await databases.listDocuments(DATABASE_ID, COLLECTIONS.TICKETS, [
          Query.equal("ticketCode", [candidate]),
          Query.limit(1),
        ]);
        if (clash.documents.length === 0) {
          ticketCodeValue = candidate;
          break;
        }
      }
      if (!ticketCodeValue) {
      ...
      const ticketCodeFinal = ticketCodeValue;
      ticket = await databases.createDocument(DATABASE_ID, COLLECTIONS.TICKETS, ID.unique(), {
        userId: authenticated.user.$id,
        eventId,
        registrationId: registration.$id,
        ticketCode: ticketCodeFinal,
        qrData: signTicket(ticketCodeFinal, eventId),
        status: "issued",
    ...
    if (registration.status === "approved") {
      ...
      if (capacity > 0 && nextRegistered < capacity) {
        ...
        if (next) {
          ...
          // Step 9: issue ticket for the promoted user (collision-checked loop).
          let ticketCodeValue = "";
          for (let attempt = 0; attempt < 5; attempt += 1) {
            const candidate = ticketCode();
            const clash = await databases.listDocuments(DATABASE_ID, COLLECTIONS.TICKETS, [
          ...
          if (ticketCodeValue) {
            const ticketCodeFinal = ticketCodeValue;
            await databases.createDocument(DATABASE_ID, COLLECTIONS.TICKETS, ID.unique(), {
              userId: next.userId,
              eventId,
