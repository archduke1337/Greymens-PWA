...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logError("Incident list error:", error);
...
export async function POST(request: NextRequest) {
  ...
  const limited = consumeRateLimit(
    `security-incident:${authenticated.user.$id}`,
    10,
    60 * 60 * 1000,
  );
  ...
  try {
    ...
    const affectedResource =
      typeof body.affectedResource === "string"
        ? body.affectedResource.trim()
        : "";
    const severity =
    ...
    if (
      ...
      !affectedResource ||
      affectedResource.length > 500 ||
      !SEVERITIES.has(severity)
    ) {
      return fail(
        "VALIDATION",
        "Title, description, affected resource, and valid severity are required",
        400,
      );
    }
    ...
    logError("Incident report error:", error);
...
export async function PATCH(request: NextRequest) {
  ...
  if (!authenticated.user) return authenticated.response;
  const limited = consumeRateLimit(
    `security-incident-decide:${authenticated.user.$id}`,
    60,
    60 * 10 * 1000,
  );
  ...
  try {
    ...
    if (!incidentId || !STATUSES.has(status))
      return fail("VALIDATION", "Invalid incident update", 400);
    if (status !== "contained") {
      const managesIncidents = await requireAnyCapability(request, [
        "security.manage_incidents",
      ]);
    ...
    if (typeof body.resolution === "string")
      data.resolution = body.resolution.trim().slice(0, 65535);
    if (typeof body.assignedTo === "string")
      data.assignedTo = body.assignedTo.trim().slice(0, 36);
    const { databases } = createServerDatabases();
    const incident = await databases.updateDocument(
      DATABASE_ID,
      COLLECTIONS.INCIDENT_REPORTS,
      incidentId,
      data,
    ...
    logError("Incident update error:", error);
