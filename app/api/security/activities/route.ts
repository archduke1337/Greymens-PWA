...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const queries = isAdmin
      ...
      : [
          Query.equal("requestedBy", [authenticated.user.$id]),
          Query.orderDesc("createdAt"),
          Query.limit(100),
        ];
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTIONS.AUTHORIZED_ACTIVITIES,
      queries,
    );

    return ok({ activities: response.documents, total: response.total });
  } catch (error) {
    logError("Authorized activity list error:", error);
...
export async function POST(request: NextRequest) {
  ...
  const limited = consumeRateLimit(
    `security-activity:${authenticated.user.$id}`,
    10,
    60 * 60 * 1000,
  );
  ...
  try {
    ...
    if (
      requiredStrings.some(
        (key) =>
          ...
          (body[key] as string).length > 65535,
      )
    ) {
      return fail(
        "VALIDATION",
        "Title, target, scope, data boundary, purpose, and time window are required",
        400,
      );
    }
    ...
    logError("Authorized activity request error:", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (!activityId || !STATUSES.has(status))
      return fail("VALIDATION", "Invalid activity decision", 400);
    const { databases } = createServerDatabases();
    ...
    logError("Authorized activity decision error:", error);
