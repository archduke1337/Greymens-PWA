...
const DB_ID = get("NEXT_PUBLIC_APPWRITE_DATABASE_ID") || "mindmesh_db";
...
// client write: security authorisations, incident reports, constitutional
// records, and role templates are sensitive governance data.
...
async function ensureIndexes(tableId, indexes) {
  ...
  const present = new Set(existing.indexes.map((index) => index.key));
  const added = [];

  for (const i of indexes) {
    if (present.has(i.key)) continue;
