...
export default function OfficesManager({
  ...
  const officeTitle = (id: string) =>
    GOVERNANCE_OFFICES.find((office) => office.id === id)?.title || id;
  ...
  const assignOffice = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Unable to assign office"));
      toast.success("Office assigned");
      setForm({ ...form, userId: "", termStart: "", termEnd: "", notes: "" });
      await onChanged();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Could not assign office",
      );
    } finally {
      setSaving(false);
    }
  ...
  const updateStatus = async (assignmentId: string, status: string) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(readApiError(payload, "Could not update office"));
      toast.success(
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
      <Card>
        <CardHeader className="flex-row items-center gap-3">
          ...
          <h2 className="text-xl font-bold">Assign office</h2>
        </CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={assignOffice}>
            <div>
              <Select
                fullWidth
                value={form.officeId}
            ...
            {membersAvailable ? (
              <div>
                <Select
                  ...
                  onChange={(value) =>
                    setForm({ ...form, userId: String(value ?? "") })
                  }
                >
                  <Label>Member</Label>
                  <Select.Trigger>
              ...
              <TextField variant="secondary">
                <Label>Appwrite user ID</Label>
                <Input
                  required
                  placeholder="Member directory unavailable — paste user ID"
                  value={form.userId}
                  onChange={(event) =>
            ...
            <div>
              <Select
                ...
                onChange={(value) =>
                  setForm({
                    ...form,
                    selectionMethod: String(value ?? "appointment"),
                  })
                }
              >
                ...
                <Select.Popover>
                  <ListBox>
                    <ListBox.Item id="election" textValue="Election">
                      Election
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    ...
                    <ListBox.Item id="interim" textValue="Interim succession">
                      Interim succession
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                  </ListBox>
            ...
            <div className="grid grid-cols-2 gap-3">
              <TextField variant="secondary">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, termStart: event.target.value })
                  }
                />
              </TextField>
              <TextField variant="secondary">
                ...
                <Input
                  ...
                  value={form.termEnd}
                  onChange={(event) =>
                    setForm({ ...form, termEnd: event.target.value })
                  }
                />
              </TextField>
            </div>
        ...
        {assignments.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center text-[var(--muted)]">
              No office assignments recorded.
            </CardContent>
          </Card>
        ) : (
          assignments.map((assignment) => {
            const name = memberName(assignment.userId);
              ...
              <Card key={assignment.$id}>
                <CardContent className="space-y-3 p-5">
                  ...
                  <p className="text-sm text-[var(--muted)]">
                    {assignment.selectionMethod} · {assignment.termStart}
