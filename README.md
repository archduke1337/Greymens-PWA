...
Club membership + events + tickets PWA for the Greymens club.
Next.js 16 / React 19 / Appwrite (client `appwrite`, server `node-appwrite`) / Tailwind 4 / Vitest.

> **Licence:** Proprietary — © 2026 Greymens Club. All rights reserved.
> This software is **not open source** (see `LICENSE`).
...
3. `npm run db:setup` — create tables/indexes from `scripts/setup-appwrite.js`.
   ...
   database; unique-index upgrades print manual remediation when duplicate rows
   block them.
4. `npm run db:seed` — idempotent upserts (`scripts/seed-data.ts`).
5. `npm run dev`

> Existing deployments that provisioned the default database before the
> Greymens rebrand keep the `mindmesh_db` database id. Either keep serving it
> by setting `NEXT_PUBLIC_APPWRITE_DATABASE_ID=mindmesh_db` explicitly, or
> re-provision under the new `greymens_db` default and migrate the data.
